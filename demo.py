...
opentrack_data_by_person[1] = 'opentrackpersonid1.csv'
opentrack_data_by_person[2] = 'opentrackpersonid2.csv'
opentrack_data_by_person[3] = 'opentrackpersonid3.csv'
...
for person_id, file_path in saved_videos.items():
    ...
    person_3d_pos = spherical_norm.spherical_to_cartesian(lon,lat,1)
    ...
    for index, row in df.iterrows():
        if part_of_start_delay == True and row['correctedTX'] == 0:
            ...
            yaw = row['correctedYaw']
            pitch = row['correctedPitch']

            #get global gaze pt -> convert back to spherical -> convert back to 2d -> save pt in list
            gaze_3d = spherical_norm.local_to_global_gaze(yaw,pitch,lon,lat)
...
while True:
    ...
    for person_id, gaze_pts_list in gaze_end_pts.items():
        ...
        if i%3==0: 
            spherical_norm.draw_arrow_global(frame,(start_x,start_y),gaze_pts_list[frame_idx], (0,0,0))
            i+=1
        elif i%3==1: 
            spherical_norm.draw_arrow_global(frame,(start_x,start_y),gaze_pts_list[frame_idx], (255,255,255))
            i+=1
        elif i%3==2: 
            spherical_norm.draw_arrow_global(frame,(start_x,start_y),gaze_pts_list[frame_idx], (0,0,255))
