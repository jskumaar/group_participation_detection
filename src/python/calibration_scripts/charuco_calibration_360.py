...
SQUARE_LENGTH = 0.036  # meters
MARKER_LENGTH = 0.029  # meters
...
print("[INFO] Press 'c' to capture manually, 'u' to undo last, 'q' to finish.")
frame_count = 0

# --- NEW: stability counter for auto-capture ---
stable_count = 0
STABLE_FRAMES_REQUIRED = 5   # adjust as needed
MIN_CORNERS_REQUIRED   = 12  # how many ChArUCo corners must be visible
...
while True:
    ...
    if len(corners) > 0:
        ...
        # Interpolate Charuco corners (needs enough markers)
        retval, charuco_corners, charuco_ids = cv2.aruco.interpolateCornersCharuco(
            corners, ids, gray, board
        )

        if retval is not None and charuco_corners is not None and charuco_ids is not None:
            # Draw interpolated chessboard corners
            cv2.aruco.drawDetectedCornersCharuco(frame, charuco_corners, charuco_ids)
            cv2.putText(frame, f"Charuco corners: {len(charuco_ids)}",
                        (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,255,0), 2)

            # --- NEW: auto-capture when stable ---
            if len(charuco_ids) >= MIN_CORNERS_REQUIRED:
                stable_count += 1
                if stable_count >= STABLE_FRAMES_REQUIRED:
                    ...
                    stable_count = 0
                    time.sleep(0.5)  # small pause to avoid duplicate captures
            else:
                stable_count = 0
        else:
            stable_count = 0
    ...
    if k == ord('c'):
        if len(corners) > 0:
            ...
            if (retval is not None and charuco_corners is not None and
                charuco_ids is not None and len(charuco_ids) > 8):
                all_corners.append(charuco_corners)
                all_ids.append(charuco_ids)
                image_size = gray.shape[::-1]
                print(f"[INFO] Captured {len(all_corners)} views (manual).")
            else:
                print("[WARN] Not enough valid Charuco corners—try a sharper/brighter view.")
        else:
            print("[WARN] No ArUco markers detected in this frame.")
...
np.savez("charuco_calib.npz", camera_matrix=camera_matrix, dist_coeffs=dist_coeffs,
         rms=rms, image_size=image_size)
print("[INFO] Saved to charuco_calib.npz")
